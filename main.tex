...
For attitude estimation in the current development phase we're planning to use a lightweight \textbf{quaternion-based complementary filter} to fuse gyroscope, accelerometer and magnetometer measurements from the IMU.

The filter implementation used in simulation consists of three steps per timestep \(\Delta t\):

\begin{enumerate}
  \item \textbf{Gyro propagation (predict):} propagate the previous quaternion \(q_{k-1}\) using measured body rates \(\boldsymbol{\omega}=[\omega_x,\omega_y,\omega_z]^\mathsf{T}\). Using the standard quaternion kinematics:
  \[
  ...
  \end{bmatrix},
  \]
  and the discrete prediction
  \[
  q_{\text{gyro}} = \frac{q_{k-1} + \tfrac{1}{2}\,\Omega(\boldsymbol{\omega})\,q_{k-1}\,\Delta t}{\big\|q_{k-1} + \tfrac{1}{2}\,\Omega(\boldsymbol{\omega})\,q_{k-1}\,\Delta t\big\|}.
  \]

  \item \textbf{Accelerometer / magnetometer correction (measurement):} compute a corrective quaternion \(q_{\text{meas}}\) from the accelerometer (roll/pitch) and magnetometer (heading). Practically this is formed by estimating roll and pitch from the accel vector and heading from the mag vector (after tilt compensation), then converting the resulting Euler angles to a quaternion:
  \[
  q_{\text{meas}} = \mathrm{eul2quat}\big(\,\widehat{\phi}_{\text{acc}},\,\widehat{\theta}_{\text{acc}},\,\widehat{\psi}_{\text{mag}}\,\big).
  \]

In the current simulation the magnetometer is not used.

  \item \textbf{Fusion (correct):} blend the fast gyro estimate with the lower-bandwidth accel/mag estimate using spherical linear interpolation (SLERP):
  \[
  q_{k} = \operatorname{slerp}\big(q_{\text{gyro}},\,q_{\text{meas}},\,t\big),\qquad t = 1-\alpha,
  \]
  where the discrete blending coefficient \(\alpha\in[0,1]\) is derived from a first-order time constant \(\tau\) via
  \[
  \alpha = \frac{\tau}{\tau + \Delta t}.
  \]
  Finally renormalize \(q_k\) to unit length.
\end{enumerate}

For the PDR simulations we use a placeholder \(\tau=0.5\ \mathrm{s}\). This \(\tau\) is an arbitrary initial choice for simulation only and will be tuned after sensor characterisation.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\linewidth]{img/example_octave_simulation_results.png}
    \caption{Example simulation results for setpoint (-10°, 10°, 40°)}
    \label{fig:octave_result}
\end{figure}
...
\begin{tikzpicture}[auto, node distance=2cm, >=Stealth]
    ...
    % Nodes
    \node[] (qref) {$q_{\mathrm{ref}}$};
    \node[draw, circle, right=1.2cm of qref, minimum size=7mm] (sum) {$\sum$};
    \node[draw, rectangle, right=1.6cm of sum, minimum width=2.0cm] (pid) {PID};
    \node[draw, rectangle, right=1.6cm of pid, minimum width=2.6cm] (plant) {FloatSat};

    % Output nodes
    \node[right=2cm of plant.north east, anchor=west] (y) {$q_{out}$};        % true output
    \node[right=2cm of plant.south east, anchor=west] (yprime) {$q_{out}'$};  % measurement to filter

    % Complementary filter block in feedback
    \node[draw, rectangle, below=1.6cm of plant, minimum width=3.0cm] (comp) {Complementary filter};

    % Arrows (forward path)
    \draw[->] (qref) -- (sum) node[midway, above] {$+$};
    \draw[->] (sum) -- (pid);
    \draw[->] (pid) -- (plant);

    % Plant outputs
    \draw[->] (plant.north east) -- (y);
    \draw[->] (plant.south east) -- (yprime);

    % Feedback: y' -> complementary filter
    \draw[->] (yprime.south) |- (comp.east) node[midway,right] {IMU measurements};

    % Feedback: complementary filter -> sum
\draw[->] (comp.west) -| (sum.south)
    node[pos=0.25, above] {$q_{\mathrm{est}}$}
    node[pos=0.75, left] {$-$};
